...
| `0052_project_deep_dives.sql` | projects | Phase 1 of Project Deep Dive tools (decision 022, `plans/active/project-deep-dives/01-schema-and-entry-points.md`): `project_deep_dives` table (owner-only RLS + `authenticated` grant + `updated_at` trigger, same shape as `projects`/`0048`) — one row per Deep Dive run, scoped to a project and a fixed `domain` (`perspectives`/`assumptions`/`research`/`implications`), `status` starts `pending`, `result` stays null until Phase 2's generation engine lands. ⚠ NOT YET APPLIED — apply before deploying the matching app code. |
